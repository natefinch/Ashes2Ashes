...
struct board_info_type board_info[NUM_OF_BOARDS] = {
  {6025, 0, 0, LVL_ASST, "etc/board.mort", 0},
  {3099, 0, 0, LVL_ASST, "etc/board.mort", 0},
  {3098, LVL_HERO, LVL_HERO, LVL_ASST, "etc/board.immort", 0},
  {3097, LVL_HERO, LVL_HERO, LVL_CIMP, "etc/board.freeze", 0},
  {3096, 0, 0, LVL_ASST, "etc/board.social", 0},
  {3095, LVL_HERO, LVL_CIMP, LVL_CIMP, "etc/board.iannounce", 0},
  {3094, 0, LVL_CIMP, LVL_CIMP, "etc/board.announce", 0},
  {3093, LVL_ASST, LVL_ASST, LVL_CIMP, "etc/board.aimp_plus", 0},
  {3092, LVL_CIMP, LVL_CIMP, LVL_CIMP, "etc/board.cimp_plus", 0},
  {3091, LVL_HERO, LVL_IMMORT, LVL_CIMP, "etc/board.coding", 0},
  {3090, LVL_HERO, LVL_IMMORT, LVL_CIMP, "etc/board.builder", 0},
  {3006, 0, 0, LVL_ASST, "etc/board.eq", 0},
};
...
void Board_write_message(int board_type, struct char_data * ch, char *arg)
{
  ...
  *(tmstr + strlen(tmstr) - 1) = '\0';
  ...
  CREATE(ch->desc->mail_to, struct mail_list, 1);
  ch->desc->mail_to->to_who = board_type + BOARD_MAGIC;
  ch->desc->mail_to->next = NULL;
...
int Board_display_msg(int board_type, struct char_data * ch, char *arg)
{
  ...
  if (!is_number(number) || (!(msg = atoi(number))))
    return 0;
