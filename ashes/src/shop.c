...
void boot_the_shops(FILE * shop_f, char *filename, int rec_count)
{
  ...
  while (!done) {
	...
	SHOP_BUYWORD(top_shop, count) = BUY_WORD(list[count]);
      ...
      if((SHOP_KEEPER(top_shop) = real_mobile(SHOP_KEEPER(top_shop))) < 0) {
        char err_buf[MAX_INPUT_LENGTH];
        sprintf(err_buf, "SYSERR: %s, no such mob", buf2);
        log(err_buf);
        SHOP_KEEPER(top_shop)=0;
      }
...
void list_detailed_shop(struct char_data * ch, int shop_nr)
{
  ...
  if (SHOP_KEEPER(shop_nr) >= 0) {
    ...
    if ((k = get_char_num(SHOP_KEEPER(shop_nr)))) {
      send_to_char(buf, ch);
      sprintf(buf, "Coins:      [%9ld], Bank: [%9d] (Total: %ld)\n\r",
