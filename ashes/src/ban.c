...
ACMD(do_ban)
{
  ...
  *buf = '\0';
  ...
  sprintf(buf, "(GC) %s has banned %s for %s players.", GET_NAME(ch), site,
	  ban_types[ban_node->type]);
  mudlog(buf, NRM, MAX(LVL_ASST, GET_LEVEL(ch)), TRUE);
...
ACMD(do_unban)
{
  ...
  sprintf(buf, "(GC) %s removed the %s-player ban on %s.",
