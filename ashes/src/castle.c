...
#define C_MOB_NUM(zone,mob) (real_mobile(zone_table[real_zone((zone))].bottom+(mob)))

#define R_MOB(zone, mob) (real_mobile(zone_table[real_zone((zone))].bottom+(mob)))
#define R_OBJ(zone, obj) (real_object(zone_table[real_zone((zone))].bottom+(obj)))
#define R_ROOM(zone, num) (real_room(zone_table[real_zone((zone))].bottom+(num)))

#define CASTLE_ITEM(item) (zone_table[real_zone(Z_KINGS_C)].bottom+(item))
...
void assign_kings_castle(void)
{
  ...
  num=C_MOB_NUM(Z_KINGS_C, 1);   /* Our dear friend, the King */
  if(num >= 0)
    mob_index[num].func=king_welmar;
  num=C_MOB_NUM(Z_KINGS_C, 3);	/* Jim */
  if(num >= 0)
    mob_index[num].func=CastleGuard;
  num=C_MOB_NUM(Z_KINGS_C, 4);	/* Brian */
  if(num >= 0)
    mob_index[num].func=CastleGuard;
  num=C_MOB_NUM(Z_KINGS_C, 5);	/* Mick */
  if(num >= 0)
    mob_index[num].func=CastleGuard;
  num=C_MOB_NUM(Z_KINGS_C, 6);	/* Matt */
  if(num >= 0)
    mob_index[num].func=CastleGuard;
  num=C_MOB_NUM(Z_KINGS_C, 7);	/* Jochem */
  if(num >= 0)
    mob_index[num].func=CastleGuard;
  num=C_MOB_NUM(Z_KINGS_C, 8);	/* Anne */
  if(num >= 0)
    mob_index[num].func=CastleGuard;
  num=C_MOB_NUM(Z_KINGS_C, 9);	/* Andrew */
  if(num >= 0)
    mob_index[num].func=CastleGuard;
  num=C_MOB_NUM(Z_KINGS_C, 10);	/* Bertram */
  if(num >= 0)
    mob_index[num].func=CastleGuard;
  num=C_MOB_NUM(Z_KINGS_C, 11);	/* Jeanette */
  if(num >= 0)
    mob_index[num].func=CastleGuard;
  num=C_MOB_NUM(Z_KINGS_C, 12);	/* Peter */
  if(num >= 0)
    mob_index[num].func=peter;
  num=C_MOB_NUM(Z_KINGS_C, 13);	/* The training master */
  if(num >= 0)
    mob_index[num].func=training_master;
  num=C_MOB_NUM(Z_KINGS_C, 16);	/* James the Butler */
  if(num >= 0)
    mob_index[num].func=James;
  num=C_MOB_NUM(Z_KINGS_C, 17);	/* Ze Cleaning Fomen */
  if(num >= 0)
    mob_index[num].func=cleaning;
  num=C_MOB_NUM(Z_KINGS_C, 20);	/* Tim, Tom's twin */
  if(num >= 0)
    mob_index[num].func=tim;
  num=C_MOB_NUM(Z_KINGS_C, 21);	/* Tom, Tim's twin */
  if(num >= 0)
    mob_index[num].func=tom;
  num=C_MOB_NUM(Z_KINGS_C, 24);	/* Dick, guard of the Treasury */
  if(num >= 0)
    mob_index[num].func=DicknDavid;
  num=C_MOB_NUM(Z_KINGS_C, 25);	/* David, Dicks brother */
  if(num >= 0)
    mob_index[num].func=DicknDavid;
  num=C_MOB_NUM(Z_KINGS_C, 26);	/* Jerry, the Gambler */
  if(num >= 0)
    mob_index[num].func=jerry;
  num=C_MOB_NUM(Z_KINGS_C, 27);	/* Michael */
  if(num >= 0)
    mob_index[num].func=CastleGuard;
  num=C_MOB_NUM(Z_KINGS_C, 28);	/* Hans */
  if(num >= 0)
    mob_index[num].func=CastleGuard;
  num=C_MOB_NUM(Z_KINGS_C, 29);	/* Boris */
  if(num >= 0)
    mob_index[num].func=CastleGuard;
}
