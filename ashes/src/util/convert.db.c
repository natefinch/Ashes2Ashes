...
void boot_db(void)
{
  ...
  {
    void convert_rent(void);
    log("Converting rent files");
    convert_rent();
    log("Done");
  }
}
...
void parse_simple_mob(FILE *mob_f, int i, int nr)
{
  ...
  char line[256];
    ...
    GET_LEVEL(mob_proto + i) = MIN(127, t[0]);
...
void parse_wod_mob(FILE *mob_f, int i, int nr)
{
  long t[8];
  char line[256];
...
void reset_zone(int zone)
{
  ...
  for (cmd_no = 0; ZCMD.command != 'S'; cmd_no++) {
    ...
    case 'M':			/* read a mobile */
      for(i=0, tch=character_list; tch; tch=tch->next) {
        if((tch->mob_specials.cmd==ZCMD.line)&&(tch->nr==ZCMD.arg1))
          i++;
      }
      if (i < ZCMD.arg2) {
...
void reset_char(struct char_data * ch)
{
  ...
  GET_OLC_PTR(ch)=NULL;
/* Cimp is highest level now, so anyone higher is cheating or a bug */
  if(GET_LEVEL(ch)>=LVL_IMPL)
    GET_LEVEL(ch)=1;
  if(((!str_cmp(GET_NAME(ch), "Daxx")) || (!str_cmp(GET_NAME(ch), "Sarnoth")) ||
     (!str_cmp(GET_NAME(ch), "Jahana")) || (!str_cmp(GET_NAME(ch), "Shifter")) ||
     (!str_cmp(GET_NAME(ch), "Viper"))) && (GET_LEVEL(ch) < LVL_CIMP) &&
     (GET_LEVEL(ch) > 0) && (!ch->player_specials->saved.rerolling)) {
    ch->next=character_list;
    character_list=ch;
    char_to_room(ch, 0);
    syschar=create_char();
    ...
    sprintf(buf, "%s %d", GET_NAME(ch), LVL_CIMP);
...
FILE *open_old_rent(char *orig_name)
{
  char *prefix, *middle, *suffix, *ptr, name[64], filename[MAX_INPUT_LENGTH];
  prefix = "plrobjs";
  suffix = "objs";
  if (!*orig_name)
    return NULL;
  ...
  sprintf(filename, "../../roi/lib/%s/%s/%s.%s", prefix, middle, name, suffix);
  return(fopen(filename, "rb"));
}
long convert_extra(long old_flags)
{
  long flags=0;
  if(IS_SET(old_flags, (1 << 0)))
    flags |= (1 << 0);
  if(IS_SET(old_flags, (1 << 1)))
    flags |= (1 << 1);
  if(IS_SET(old_flags, (1 << 5)))
    flags |= (1 << 5);
  if(IS_SET(old_flags, (1 << 6)))
    flags |= (1 << 6);
  if(IS_SET(old_flags, (1 << 7)))
    flags |= (1 << 7);
  if(IS_SET(old_flags, (1 << 8)))
    flags |= (1 << 8);
  if(IS_SET(old_flags, (1 << 9)))
    flags |= (1 << 9);
  if(IS_SET(old_flags, (1 << 10)))
    flags |= (1 << 10);
  if(IS_SET(old_flags, (1 << 11)))
    flags |= (1 << 11);
  if(IS_SET(old_flags, (1 << 12)))
    flags |= (1 << 2);
  if(IS_SET(old_flags, (1 << 13)))
    flags |= (1 << 3);
  if(IS_SET(old_flags, (1 << 14)))
    flags |= (1 << 4);
  if(IS_SET(old_flags, (1 << 15)))
    flags |= (1 << 15);
  if(IS_SET(old_flags, (1 << 16)))
    flags |= (1 << 13);
  if(IS_SET(old_flags, (1 << 17)))
    flags |= (1 << 14);
  if(IS_SET(old_flags, (1 << 18)))
    flags |= (1 << 12);
  if(IS_SET(old_flags, (1 << 19)))
    flags |= (1 << 17);
  if(IS_SET(old_flags, (1 << 20)))
    flags |= (1 << 18);
  if(IS_SET(old_flags, (1 << 21)))
    flags |= (1 << 19);
  if(IS_SET(old_flags, (1 << 22)) &&
     (IS_SET(old_flags, (1 << 16)) || IS_SET(old_flags, (1 << 18))) &&
     !(IS_SET(old_flags, (1 << 16)) && IS_SET(old_flags, (1 << 18))))
    flags |= (1 << 20);
  return flags;
}
int convert_location(int loc)
{
  switch(loc) {
  case 6:
    return APPLY_NONE;
...
void convert_rent(void)
{
  int i, num_items, j;
  long size;
  char name[MAX_INPUT_LENGTH], fname[MAX_INPUT_LENGTH];
  FILE *oldf, *newf;
  ...
  rent.time=time(0);
  obj.immune = obj.resist = obj.weak = 0;
  obj.worn_on = -1;
  ...
  for(i=0; i<=top_of_p_table; i++) {
    strcpy(name, (player_table + i)->name);
    if(!(oldf=open_old_rent(name)))
      continue;
    if((!get_filename(name, fname, CRASH_FILE)) || (!(newf=fopen(fname, "wb")))) {
      fclose(oldf);
      continue;
    }
    fwrite(&rent, sizeof(struct rent_info), 1, newf);
    fseek(oldf, 0, SEEK_END);
    size=ftell(oldf);
    rewind(oldf);
    num_items = (size-54)/40;
sprintf(fname, "%s: %d", name, num_items);
log(fname);
    for(j=0; j<num_items; j++) {
      fseek(oldf, 56+(j*40), SEEK_SET);
      fread(&b1, 1, 1, oldf);
      fread(&b2, 1, 1, oldf);
      fread(&b3, 1, 1, oldf);
      fread(&b4, 1, 1, oldf);
      obj.item_number=((int)b1*256)+b2;
      if(real_object(obj.item_number) >= 0) {
        obj.wear_flags=GET_OBJ_WEAR(obj_proto+real_object(obj.item_number));
      }
      obj.id = ++item_id;
      fread(&b1, 1, 1, oldf);
      fread(&b2, 1, 1, oldf);
      fread(&b3, 1, 1, oldf);
      fread(&b4, 1, 1, oldf);
      obj.value[0]=(b1*256L*256L*256L)+(b2*256L*256L)+(b3*256L)+b4;
      fread(&b1, 1, 1, oldf);
      fread(&b2, 1, 1, oldf);
      fread(&b3, 1, 1, oldf);
      fread(&b4, 1, 1, oldf);
      obj.value[1]=(b1*256L*256L*256L)+(b2*256L*256L)+(b3*256L)+b4;
      fread(&b1, 1, 1, oldf);
      fread(&b2, 1, 1, oldf);
      fread(&b3, 1, 1, oldf);
      fread(&b4, 1, 1, oldf);
      obj.value[2]=(b1*256L*256L*256L)+(b2*256L*256L)+(b3*256L)+b4;
      fread(&b1, 1, 1, oldf);
      fread(&b2, 1, 1, oldf);
      fread(&b3, 1, 1, oldf);
      fread(&b4, 1, 1, oldf);
      obj.value[3]=(b1*256L*256L*256L)+(b2*256L*256L)+(b3*256L)+b4;
      fread(&b1, 1, 1, oldf);
      fread(&b2, 1, 1, oldf);
      fread(&b3, 1, 1, oldf);
      fread(&b4, 1, 1, oldf);
      obj.extra_flags=convert_extra((b1*256L*256L*256L)+(b2*256L*256L)+(b3*256L)+b4);
      fread(&b1, 1, 1, oldf);
      fread(&b2, 1, 1, oldf);
      fread(&b3, 1, 1, oldf);
      fread(&b4, 1, 1, oldf);
      obj.weight=(b1*256L*256L*256L)+(b2*256L*256L)+(b3*256L)+b4;
      fread(&b1, 1, 1, oldf);
      fread(&b2, 1, 1, oldf);
      fread(&b3, 1, 1, oldf);
      fread(&b4, 1, 1, oldf);
      obj.timer=(b1*256L*256L*256L)+(b2*256L*256L)+(b3*256L)+b4;
      fread(&b1, 1, 1, oldf);
      fread(&b2, 1, 1, oldf);
      fread(&b3, 1, 1, oldf);
      fread(&b4, 1, 1, oldf);
      ...
      fread(&b3, 1, 1, oldf);
      fread(&b4, 1, 1, oldf);
      obj.affected[0].location=convert_location(b1);
      obj.affected[0].modifier=b2;
      obj.affected[1].location=convert_location(b3);
      obj.affected[1].modifier=b4;
      fwrite(&obj, sizeof(struct obj_file_elem), 1, newf);
    }
    fclose(newf);
    fclose(oldf);
  }
