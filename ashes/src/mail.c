			  ...
			  int cmd, char *arg)
{
  ...
  while(*buf) {
    if ((recipient = get_id_by_name(buf)) < 0) {
      sprintf(buf2, "$n tells you, 'No one by the name %s is registered here!'", buf);
      act(buf2, FALSE, mailman, 0, ch, TO_VICT);
      while(ch->desc->mail_to) {
        newmail=ch->desc->mail_to;
        ch->desc->mail_to=newmail->next;
        free(newmail);
      }
      return;
    }
    CREATE(newmail, struct mail_list, 1);
    newmail->to_who=recipient;
    newmail->next=ch->desc->mail_to;
    ch->desc->mail_to=newmail;
    if (load_char(buf, &tmp_store) > -1) {
      if(tmp_store.level < LVL_HERO)
        cost+=STAMP_PRICE;
    }
    arg=one_argument(arg, buf);
  }

  if((GET_GOLD(ch) < cost) && (GET_LEVEL(ch) < LVL_HERO)) {
    sprintf(buf, "$n tells you, 'A stamp costs %d coins. You need one for each mortal you mail.'\r\n"
            "$n tells you, 'That comes to %d coins, which I see you can't afford.'\r\n",
            STAMP_PRICE, cost);
    act(buf, FALSE, mailman, 0, ch, TO_VICT);
    while(ch->desc->mail_to) {
      newmail=ch->desc->mail_to;
  ...
  if(GET_LEVEL(ch) < LVL_HERO) {
    sprintf(buf, "$n tells you, 'I'll take %d coins for the stamp%s.'\r\n"
         ...
         "$n tells you, 'Use %% on a new line to cancel the mail.'",
         cost, ((cost>STAMP_PRICE)?"s":""));
    GET_GOLD(ch) -= cost;
  }
  else {
    strcpy(buf, "$n tells you, 'No charge for you, use @ on a new line when done.'\r\n"
                "$n tells you, 'Use % on a new line to cancel the mail.'");
  }
